...
import pt.uminho.algoritmi.netopt.nfv.optimization.jecoli.evaluation.NFVEvaluationServIGP;
...
public class JecoliServIGP
{
    ...
    public JecoliServIGP(NetworkTopology topology, NFVState state, int lowerBound, int upperBound, String file, int maxServicesPenalization, int cplexTimeLimit) {
    ...
    public ILinearRepresentationFactory<Integer> configureSolutionFactory (int numberServices, int numberEdges, int numberOfObjectives)
    {	int numberVariables =numberServices+numberEdges;
        ArrayList<Integer> l=new ArrayList<Integer>();
        ArrayList<Integer> u=new ArrayList<Integer>();
        for(int i=0;i<numberServices;i++){
            l.add(i,MINServicesSolutions);u.add(i,MAXServicesSolutions);
        }
        for(int i=numberServices;i<numberVariables;i++){
    ...
    private ISolutionSet<ILinearRepresentation<Integer>> buildInitialPopulation(ParamsNFV params,
        ...
        if (p > 0) {
            ...
            while (it.hasNext())
                newSolutionSet.add(pt.uminho.algoritmi.netopt.ospf.optimization.jecoli.SolutionParser.convert(it.next(), this.NUMObjectives));
        }
    ...
    public NSGAIIConfiguration<ILinearRepresentation<Integer>, ILinearRepresentationFactory<Integer>> preConfigureNSGAII(
        ...
        ILinearRepresentationFactory solutionFactory = configureSolutionFactory(topology.getDimension(),topology.getNumberEdges(),2);
    ...
    public void configureNSGAII(ParamsNFV params) throws Exception, InvalidConfigurationException {
        ...
        NFVEvaluationServIGP nfvEvaluation = new NFVEvaluationServIGP(topology,state, servicesConfiguration, this.maxServicesPenalization, this.cplexTimeLimit);
