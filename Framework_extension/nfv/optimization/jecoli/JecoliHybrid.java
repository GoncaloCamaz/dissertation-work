...
public class JecoliHybrid
{
    ...
    private int MAXIGPWeights;
    private int MINIGPWeights;
    private double alpha;
    private int cplexTimeLimit;

    public JecoliHybrid(NetworkTopology topology, NFVState state, int lowerBoundConf, int upperBoundConf, int lowerBoundIGP, int upperBoundIGP, String file, int cplexTimeLimit, double alpha) {
        ...
        this.MINServicesSolutions = lowerBoundConf;
        this.MAXServicesSolutions = upperBoundConf;
        this.MAXIGPWeights = upperBoundIGP;
        this.MINIGPWeights = lowerBoundIGP;
    ...
    public ILinearRepresentationFactory<Integer> configureSolutionFactoryWP (int numberNodes, int numberEdges, int numberOfObjectives)
    {
        ...
        for(int i=0;i<numberNodes;i++){
            l.add(i,this.MINServicesSolutions);u.add(i,this.MAXServicesSolutions);
        }
        for(int i=numberNodes;i<numberVariables;i++){
            l.add(i,MINIGPWeights);u.add(i,this.MAXIGPWeights);
        }
    ...
    public void configureNSGAII(ParamsNFV params) throws Exception, InvalidConfigurationException {
        ...
        HybridEvaluation evaluation = new HybridEvaluation(topology,state,servicesConfiguration,cplexTimeLimit,params.getAlgorithm(),this.alpha);
